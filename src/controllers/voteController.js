...
exports.getVotesByCampaignId = async (req, res) => {
    ...
    try {
        ...
        if (votes.length === 0) {
            return res.status(404).json({ message: "No votes found for this campaign" });
        }
...
exports.createVote = async (req, res) => {
    ...
    try {
        const newVote = await Votes.create({ campaignId: campaignId, userId: userId, voteType: vote });
        if (!newVote) {
...
exports.getVoteByUserIdAndCampaignId = async (req, res) => {
    ...
    try {
        ...
        if (!vote) {
            return res.status(404).json({ message: "Vote not found for this user and campaign" });
        }
